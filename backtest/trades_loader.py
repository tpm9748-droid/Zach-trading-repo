...
_TRADES_COLS = ["ts_event", "action", "price", "size", "symbol"]
...
def _agg_frame(df: pd.DataFrame) -> pd.DataFrame:
    """Group a (already symbol-filtered, trades-only) frame to 1m OHLCV.

    Expects a `ts` column floored to the minute. Returns columns
    ts/open/high/low/close/volume, sorted by ts.
    """
    g = (
        df.groupby("ts")
        .agg(
...
def aggregate_trades_to_1m_bars(df: pd.DataFrame, symbol: str) -> list[Bar]:
    ...
    return _frame_to_bars(_agg_frame(sub))
